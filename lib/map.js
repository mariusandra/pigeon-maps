...
var Map = function (_Component) {
  ...
  function Map(props) {
    ...
    _this.handleTouchStart = function (event) {
      ...
      if (event.touches.length === 1) {
        ...
        if (coords[0] >= 0 && coords[1] >= 0 && coords[0] < width && coords[1] < height) {
          ...
          _this.startTrackingMoveEvents(coords);
    ...
    _this.handleTouchEnd = function (event) {
      if (_this._touchStartCoords) {
        ...
        if (event.touches.length === 0) {
          ...
          var coords = getMouseCoords(_this._containerRef, event.changedTouches[0]);
          _this.throwAfterMoving(coords, center, zoom);
        } else if (event.touches.length === 1) {
          var touch = event.touches[0];
          var _coords = getMouseCoords(_this._containerRef, touch);

          _this._touchStartCoords = [[touch.clientX, touch.clientY]];
          _this.startTrackingMoveEvents(_coords);
        }
      }
    };
    ...
    _this.syncToProps = function () {
      ...
      if (onBoundsChanged) {
        var bounds = {
          ne: _this.pixelToLatLng(width - 1, 0, zoom),
          sw: _this.pixelToLatLng(0, height - 1, zoom)
        };
