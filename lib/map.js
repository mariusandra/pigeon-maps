...
var Map = function (_Component) {
  ...
  function Map(props) {
    ...
    _this.handleTouchStart = function (event) {
      ...
      if (event.touches.length === 1) {
        ...
        if (_this.state.centerDelta || _this.state.zoomDelta) {
    ...
    _this.handleTouchMove = function (event) {
      if (event.touches.length === 1 && _this._touchStartCoords) {
        ...
        _this.setState({
          centerDelta: [touch.clientX - _this._touchStartCoords[0][0], touch.clientY - _this._touchStartCoords[0][1]]
        });
      } else if (event.touches.length === 2 && _this._touchStartCoords) {
        ...
        _this.setState({
          zoomDelta: zoomDelta,
          centerDelta: [centerDiffDiff[0] + midPointDiff[0] * scale, centerDiffDiff[1] + midPointDiff[1] * scale]
        });
      }
    };
    ...
    _this.handleMouseDown = function (event) {
      ...
      if (coords[0] >= 0 && coords[1] >= 0 && coords[0] < width && coords[1] < height) {
        ...
        event.preventDefault();
        _this._mouseEvents = [{ timestamp: window.performance.now(), coords: coords }];
      }
    };
    ...
    _this.handleMouseMove = function (event) {
      ...
      if (_this._mouseDown && _this._dragStart) {
        ...
        if (timestamp - _this._mouseEvents[_this._mouseEvents.length - 1].timestamp > 40) {
          _this._mouseEvents.push({ timestamp: timestamp, coords: _this._mousePosition });
          if (_this._mouseEvents.length > 2) {
            _this._mouseEvents.shift();
          }
        }

        _this.setState({
          centerDelta: [_this._mousePosition[0] - _this._dragStart[0], _this._mousePosition[1] - _this._dragStart[1]]
        });
      }
    };
    ...
    _this.handleMouseUp = function (event) {
      ...
      if (_this._mouseDown) {
        ...
        var lastEvent = _this._mouseEvents.shift();

        if (lastEvent) {
          var deltaMs = Math.max(window.performance.now() - lastEvent.timestamp, 1);
    ...
    _this.sendDeltaChange = function () {
      var _this$state = _this.state,
          ...
          centerDelta = _this$state.centerDelta,
  ...
  _createClass(Map, [{
    ...
    value: function tileValues(props, state) {
      ...
      var tileCenterX = lng2tile(center[1], roundedZoom) - (centerDelta ? centerDelta[0] / 256.0 / scale : 0);
      var tileCenterY = lat2tile(center[0], roundedZoom) - (centerDelta ? centerDelta[1] / 256.0 / scale : 0);
    ...
    value: function renderTiles() {
      ...
      for (var _x7 = xMin; _x7 <= xMax; _x7++) {
        for (var _y = yMin; _y <= yMax; _y++) {
          tiles.push({
            key: _x7 + '-' + _y + '-' + roundedZoom,
            url: mapUrl(_x7, _y, roundedZoom),
            left: (_x7 - tileMinX) * 256,
            top: (_y - tileMinY) * 256,
            width: 256,
            height: 256,
            active: true
    ...
    value: function renderOverlays() {
      ...
      var _state = this.state,
          zoom = _state.zoom,
          centerDelta = _state.centerDelta,
          zoomDelta = _state.zoomDelta;
      ...
      var childrenWithProps = _react2.default.Children.map(this.props.children, function (child) {
        ...
        if (position) {
          var c = _this3.latLngToPixel(position[0], position[1], zoom + zoomDelta);
