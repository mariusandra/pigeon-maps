  ...
  function Map(props) {
    ...
    _this.handleTouchStart = function (event) {
      ...
      if (event.touches.length === 1) {
        ...
        if (_this.state.dragDelta || _this.state.zoomDelta) {
    ...
    _this.handleTouchMove = function (event) {
      if (event.touches.length === 1 && _this._touchStartCoords) {
        ...
        _this.setState({
          zoomDelta: zoomDelta,
          dragDelta: [centerDiffDiff[0] + midPointDiff[0] * scale, centerDiffDiff[1] + midPointDiff[1] * scale]
        });
      }
    };
    ...
    _this.sendDeltaChange = function () {
      var _this$props4 = _this.props,
          ...
          onBoundsChanged = _this$props4.onBoundsChanged;
      var _this$state = _this.state,
          dragDelta = _this$state.dragDelta,
          zoomDelta = _this$state.zoomDelta;


      if (onBoundsChanged && (dragDelta || zoomDelta !== 0)) {
        var lng = tile2lng(lng2tile(center[1], zoom + zoomDelta) - (dragDelta ? dragDelta[0] / 256.0 : 0), zoom + zoomDelta);
        var lat = tile2lat(lat2tile(center[0], zoom + zoomDelta) - (dragDelta ? dragDelta[1] / 256.0 : 0), zoom + zoomDelta);
    ...
    _this.zoomAroundMouse = function (zoomDiff) {
      var _this$props7 = _this.props,
          center = _this$props7.center,
          zoom = _this$props7.zoom,
          onBoundsChanged = _this$props7.onBoundsChanged;
  ...
  _createClass(Map, [{
    ...
    value: function renderOverlays() {
      ...
      var childrenWithProps = _react2.default.Children.map(this.props.children, function (child) {
        ...
        if (position) {
          var c = _this4.latLngToPixel(position[0], position[1], zoom + zoomDelta);
