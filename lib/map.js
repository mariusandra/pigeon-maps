...
var SCROLL_PIXELS_FOR_ZOOM_LEVEL = 50;
  ...
  function Map(props) {
    ...
    _this.setCenterZoomTarget = function (center, zoom, fromProps) {
      ...
      if (_this.props.animate) {
        ...
        _this._animationEnd = _this._animationStart + ANIMATION_TIME;
    ...
    _this.animationStep = function (timestamp) {
      ...
      var percentage = progress / length;
    ...
    _this.sendDeltaChange = function () {
      ...
      if (centerDelta || zoomDelta !== 0) {
        var lng = tile2lng(lng2tile(center[1], zoom + zoomDelta) - (centerDelta ? centerDelta[0] / 256.0 : 0), zoom + zoomDelta);
        var lat = tile2lat(lat2tile(center[0], zoom + zoomDelta) - (centerDelta ? centerDelta[1] / 256.0 : 0), zoom + zoomDelta);
    ...
    _this.handleWheel = function (event) {
      event.preventDefault();
      _this.zoomAroundMouse(-event.deltaY / SCROLL_PIXELS_FOR_ZOOM_LEVEL);
    };
